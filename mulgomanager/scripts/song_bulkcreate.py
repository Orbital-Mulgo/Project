...
def run():
    ...
    print("Stage 1: Songs Dictionary Conversion Starts at %s seconds" %
    ...
    full_songs_list['release_date'] = full_songs_list['release_date'].astype(object).where(
        full_songs_list['release_date'].notnull(), None)
    ...
    print("Stage 1: Songs Dictionary Conversion Completed at %s seconds" %
          (str(timedelta(seconds=time.time() - start_time)).split(".")[0]))
    print("Stage 2: Songs Model Creation Starts at %s seconds" %
          (str(timedelta(seconds=time.time() - start_time)).split(".")[0]))
    ...
    print("Stage 2: Songs Model Creation Completed at %s seconds" %
          (str(timedelta(seconds=time.time() - start_time)).split(".")[0]))

    artist_counter, song_with_artist = 0, 0
    # song_counter = 0
    missing_song, missing_artist = [], []
    print("Stage 3: Artists Addition Starts at %s seconds" %
          (str(timedelta(seconds=time.time() - start_time)).split(".")[0]))
    ...
    print("Stage 3: Artists Addition Completed at %s seconds" %
          (str(timedelta(seconds=time.time() - start_time)).split(".")[0]))
    ...
    print("--- Database updated with %d songs in %s seconds ---" %
          (number_of_songs, str(timedelta(seconds=time.time() - start_time)).split(".")[0]))
