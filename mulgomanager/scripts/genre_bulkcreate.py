...
def run():
    ...
    print("Stage 1: Genres Dictionary Conversion Starts at %s seconds" %
    ...
    print("Stage 1: Genres Dictionary Conversion Completed at %s seconds" %
          (str(timedelta(seconds=time.time() - start_time)).split(".")[0]))
    print("Stage 2: Genres Model Creation Starts at %s seconds" %
          (str(timedelta(seconds=time.time() - start_time)).split(".")[0]))
    ...
    print("Stage 2: Genres Model Creation Completed at %s seconds" %
          (str(timedelta(seconds=time.time() - start_time)).split(".")[0]))
    print("--- Database updated with %d genres in %s seconds ---" %
          (number_of_genres, str(timedelta(seconds=time.time() - start_time)).split(".")[0]))
